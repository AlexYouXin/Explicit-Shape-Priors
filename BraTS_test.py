...
from dataloader.dataset_brats import brats_dataset
from utils.BraTS_utils import test_single_volume
...
parser.add_argument('--volume_path', type=str,
                    default='/lustre/home/acct-medcb/medcb-cb1/yx/medical data/BraTS 2020/train', help='root dir for validation volume data')  # for acdc volume_path=root_dir
parser.add_argument('--edge_path', type=str,
                    default='/lustre/home/acct-medcb/medcb-cb1/yx/medical data/BraTS 2020/train/edge', help='edge dir for train and val data')
...
parser.add_argument('--img_size', type=int, default=[128, 128, 128], help='input patch size of network input')
parser.add_argument('--n_gpu', type=int, default=2, help='total gpu')
parser.add_argument('--is_savenii', action="store_true", default='True', help='whether to save results during inference')
parser.add_argument('--threshold', type=float, default=0.50, help='using number of skip-connect, default is num')
...
def inference(args, model0, model1, model2, test_save_path=None):
    ...
    area_dice_case = np.array((0, 0, 0)).astype(np.float64)
    area_hd_case = np.array((0, 0, 0)).astype(np.float64)
    # threshold = np.linspace(start = 0.5, stop = 0.7, num = 11)
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        metric_i, index_i, binary_metric_i, time_i = test_single_volume(image, label, model0, model1, model2, classes=args.num_classes, patch_size=args.img_size,         # maybe changed into the x and y dimensions
                                      test_save_path=test_save_path, case=case_name, origin=origin, spacing=spacing, threshold=args.threshold)
        ...
        logging.info('idx: %d, case: %s, ET dice: %f, ET hd95: %f' % (i_batch, case_name, metric_i[1, 0], metric_i[1, 1]))
        logging.info('idx: %d, case: %s, TC dice: %f, TC hd95: %f' % (i_batch, case_name, metric_i[2, 0], metric_i[2, 1]))
        logging.info('idx: %d, case: %s, WT dice: %f, WT hd95: %f' % (i_batch, case_name, metric_i[3, 0], metric_i[3, 1]))
...
if __name__ == "__main__":
    ...
    dataset_config = {
        'BraTS': {
            ...
            'volume_path': '/lustre/home/acct-eeyj/eeyj-wr/youxin/medical_dataset/BraTS2020/train',
    ...
    if dataset_name == 'BraTS':  # using max_epoch instead of iteration to control training duration
    ...
    net0 = network(in_channel=4, out_channel=args.num_classes, training=False, config=config_vit).cuda()
    net1 = network(in_channel=4, out_channel=args.num_classes, training=False, config=config_vit).cuda()
    net2 = network(in_channel=4, out_channel=args.num_classes, training=False, config=config_vit).cuda()
    ...
    snapshot1 = os.path.join(snapshot_path, 'epoch_2000.pth')  
    snapshot2 = os.path.join(snapshot_path, 'epoch_1950.pth')
