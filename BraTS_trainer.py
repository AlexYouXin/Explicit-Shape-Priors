...
from utils.BraTS_utils import DiceLoss, print_network, DiceLoss_area, cross_loss_area
...
def train(model, writer, optimizer, dice_weight, ce_weight, args, epoch_num, trainloader, snapshot_path):
    ...
    ce_loss = cross_loss_area(num_classes, dice_weight)
    dice_loss = DiceLoss_area(num_classes, dice_weight)
...
def run_main(args, model, snapshot_path):
    from dataloader.dataset_brats import brats_dataset, RandomGenerator
    ...
    # train dataset
    db_train = brats_dataset(base_dir=args.train_root_path, list_dir=args.list_dir, split="train", num_classes=args.num_classes,
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=args.img_size, mode = 'train')]))

    print("The length of train set is: {}".format(len(db_train)))
    # val dataset
    db_val = brats_dataset(base_dir=args.val_root_path, list_dir=args.list_dir, split="val", num_classes=args.num_classes,
                               transform=transforms.Compose(
    ...
    optimizer = optim.AdamW(model.parameters(), lr=8e-4, eps=1e-8, betas=(0.9, 0.999), weight_decay=1e-5)
    ...
    dice_weight = [0.20] + [1.40] + [1.40] + [1.00]
    ce_weight = [0.05] + [0.35] + [0.35] + [0.25]
